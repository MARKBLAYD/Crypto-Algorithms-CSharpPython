...
class Program
{
    static void Main()
    {
        ...
        {
            Console.Clear();
            Console.WriteLine("1. Вычислить хеш Стрибог-512");
            Console.WriteLine("2. Вычислить хеш Стрибог-256");
            ...
            try
            {
                ...
                if (choice == 1)
                {
                    ...
                    hash = G512.GetHash(fileData);
                    SaveHashToFile(hash, "512hesh_", filePath);
                }
                else
                {
                    GOST G256 = new GOST(256);
                    hash = G256.GetHash(fileData);
                    SaveHashToFile(hash, "256hesh_", filePath);
                }

                Console.WriteLine("Хеш успешно вычислен и записан в файл.");
            }
            catch (Exception ex)
            {
    ...
    static void SaveHashToFile(byte[] hash, string prefix, string originalFilePath)
    {
        string fileName = Path.GetFileName(originalFilePath);
        string folder = Path.GetDirectoryName(originalFilePath);
        string resultPath = Path.Combine(folder, prefix + fileName);

        string hashHex = BitConverter.ToString(hash).Replace("-", "").ToLower();
        File.WriteAllText(resultPath, hashHex);
        Console.WriteLine($"Хеш сохранен в файл: {resultPath}");
    }
}
